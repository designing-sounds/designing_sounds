...
def get_fft(k: np.array) -> Tuple[List[Tuple[float, float]], float]:
    samples = len(k)
...
class SoundModel:
    ...
    def remove_power_spectrum(self, index: int) -> None:
        with self.lock:
            ...
            for i in range(index, len(self.__power_spectrum.num_harmonics_per_spectrum) - 1):
                self.__power_spectrum.num_harmonics_per_spectrum[i] = self.__power_spectrum.num_harmonics_per_spectrum[
                    i + 1]
    ...
    def get_power_spectrum_graph(self, power_spectrum_idx: int, samples: int) -> Tuple[
    ...
    def get_sum_all_power_spectrums_graph(self, samples: int) -> Tuple[List[Tuple[float, float]], float]:
        with self.lock:
            x = np.linspace(0, 1, samples)
            k = np.zeros(len(x))
    ...
    def interpolate_points(self, points: typing.List[typing.Tuple[float, float]], update_noise: bool = False) -> None:
        with self.lock:
    ...
    def update_all_power_spectrums(self, freqs: np.ndarray, periodic_sd: float, periodic_lengthscale: float,
                                   squared_sd: float, squared_lengthscale: float, nums_harmonic: int,
                                   points: typing.List[typing.Tuple[float, float]]) -> None:
        with self.lock:
    ...
    def model_sound(self, sample_rate: int, chunk_duration: float, start_time: float) -> np.ndarray:
        ...
        with self.lock:
            sound = self.__power_spectrum.prior.prior(x, self.__power_spectrum.get_freqs(),
                                                      self.__power_spectrum.get_periodic_sds(),
                                                      self.__power_spectrum.get_periodic_lengthscales(),
    ...
    def update_train_prior(self) -> None:
    ...
    def update(self, x_test: np.ndarray) -> np.ndarray:
        return self.matrix_covariance(x_test, self.x_train) @ self.inv @ (self.y_train - self.noise - self.train_prior)

    def matrix_covariance(self, x_1: np.ndarray, x_2: np.ndarray) -> np.ndarray:
        return np.sum(
